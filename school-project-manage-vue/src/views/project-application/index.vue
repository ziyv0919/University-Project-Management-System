<template>
	<div class="min-h-full mx-auto p-6">
		<div class="header">
			<div class="header-left-toolbar">
				<el-button round :icon="Plus" @click="addApplication">新增申请</el-button>
			</div>
			<div class="header-right-search">
				<el-popover :width="400" :visible="visible" :teleported="false" placement="bottom-start">
					...
					<el-form label-position="top" label-width="100px" :model="pagination" @keyup.enter="searchData">
						...
						<el-form-item label="申请学生" prop="uId">
							<el-select
		...
		<el-table :data="applicationData" style="width: 100%" border stripe v-loading="loading" @row-dblclick="handleRowDblclick">
	...
	<!-- 新增/编辑申请对话框 -->
	<el-dialog v-model="dialogVisible" :title="isEdit ? '编辑申请' : '新增申请'" width="500px">
		<el-form ref="applicationFormRef" :model="applicationForm" :rules="applicationRules" label-width="100px">
			<el-form-item label="课题" prop="pId">
				<el-select
					v-model="applicationForm.pId"
					filterable
					remote
					reserve-keyword
					placeholder="请输入课题名称搜索"
			...
			<el-form-item label="学生" prop="uId">
				<el-select
					v-model="applicationForm.uId"
					filterable
					remote
					reserve-keyword
					placeholder="请输入学生姓名搜索"
...
import {
	postProjectApplicationsInsert,
	postProjectApplicationsUpdate,
...
const studentOptions = ref<Array<{ value: number; label: string }>>([]);
...
// 远程搜索学生
async function remoteSearchStudents(query: string) {
	if (query) {
...
const applicationRules = reactive<FormRules>({
	pId: [{ required: true, message: '请选择课题', trigger: 'change' }],
	uId: [{ required: true, message: '请选择学生', trigger: 'change' }],
});
...
async function approveApplication(row: any, status: number) {
	...
	ElMessageBox.confirm(`确定要${statusText}申请编号为 ${row.paId} 的申请吗？`, '提示', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		...
		.then(async () => {
			try {
				...
				await postProjectApplicationsApprove(params);
				ElMessage.success(`申请${statusText}成功`);
				searchData();
			} catch (error) {
				ElMessage.error(`申请${statusText}失败`);
			}
		})
		.catch(() => {});
}
		...
		if (valid) {
			...
			try {
				if (isEdit.value) {
					// 编辑申请
					const params = filterEmptyParams({
						paId: applicationForm.paId,
						pId: applicationForm.pId,
						uId: applicationForm.uId,
					});
					await postProjectApplicationsUpdate(params);
					...
					await postProjectApplicationsInsert(params);
...
// 重置表单
function resetForm() {
	applicationForm.paId = undefined;
	applicationForm.pId = undefined;
	applicationForm.uId = undefined;
